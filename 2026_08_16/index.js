...
(function () {
  ...
  function openFullEditorModal() {
    ...
    caseButton.className = 'editor-find-button';
    caseButton.textContent = 'Aa';
    caseButton.title = 'Diferenciar maiúsculas e minúsculas';
    caseButton.setAttribute('aria-pressed', 'false');
    var previousButton = document.createElement('button');
    previousButton.type = 'button';
    previousButton.className = 'editor-find-button';
    previousButton.textContent = '↑';
    previousButton.title = 'Ocorrência anterior';
    previousButton.setAttribute('aria-label', 'Ocorrência anterior');
    var nextButton = document.createElement('button');
    nextButton.type = 'button';
    nextButton.className = 'editor-find-button';
    nextButton.textContent = '↓';
    nextButton.title = 'Próxima ocorrência';
    nextButton.setAttribute('aria-label', 'Próxima ocorrência');
    var replaceButton = document.createElement('button');
    replaceButton.type = 'button';
    replaceButton.className = 'editor-find-button';
    replaceButton.textContent = 'Substituir';
    var replaceAllButton = document.createElement('button');
    replaceAllButton.type = 'button';
    replaceAllButton.className = 'editor-find-button';
    replaceAllButton.textContent = 'Todos';
    ...
    function rebuildMatches(preferredPosition) {
      matchPositions = [];
      var query = findInput.value;
      if (query) {
        var source = isCaseSensitive() ? textarea.value : textarea.value.toLocaleLowerCase();
        var needle = isCaseSensitive() ? query : query.toLocaleLowerCase();
        var from = 0;
      ...
      if (matchPositions.length) {
        var position = typeof preferredPosition === 'number' ? preferredPosition : textarea.selectionStart;
        for (var i = 0; i < matchPositions.length; i++) {
          if (matchPositions[i] >= position) { currentMatch = i; break; }
        }
    ...
    function updateSearchStatus() {
      matchStatus.textContent = findInput.value ? (matchPositions.length ? (currentMatch + 1) + ' de ' + matchPositions.length : 'Nenhum') : '';
      var disabled = !matchPositions.length;
    ...
    function selectCurrentMatch() {
      if (currentMatch < 0 || !matchPositions.length) return;
      var start = matchPositions[currentMatch];
      textarea.focus();
      textarea.setSelectionRange(start, start + findInput.value.length);
    ...
    function scrollMatchIntoView(start) {
      ...
      if (!isFinite(lineHeight)) lineHeight = parseFloat(styles.fontSize) * 1.35;
      var line = (textarea.value.slice(0, start).match(/\n/g) || []).length;
      var matchTop = line * lineHeight;
      var visibleTop = textarea.scrollTop;
      var visibleBottom = visibleTop + textarea.clientHeight - lineHeight;
      if (matchTop < visibleTop || matchTop > visibleBottom) {
        textarea.scrollTop = Math.max(0, matchTop - (textarea.clientHeight - lineHeight) / 2);
      }
    }
    function moveMatch(step) {
      if (!matchPositions.length) return;
      currentMatch = (currentMatch + step + matchPositions.length) % matchPositions.length;
      updateSearchStatus();
      selectCurrentMatch();
    }
    function replaceCurrent() {
      if (currentMatch < 0 || !matchPositions.length) return;
      var start = matchPositions[currentMatch];
      textarea.setRangeText(replaceInput.value, start, start + findInput.value.length, 'end');
      rebuildMatches(start + replaceInput.value.length);
      selectCurrentMatch();
    }
    function replaceAll() {
      if (!matchPositions.length) return;
      ...
      for (var i = matchPositions.length - 1; i >= 0; i--) {
        var start = matchPositions[i];
        textarea.setRangeText(replacement, start, start + findInput.value.length, 'preserve');
      }
      var total = matchPositions.length;
      rebuildMatches(0);
      showLocalEditorNotification(total + (total === 1 ? ' ocorrência substituída.' : ' ocorrências substituídas.'), 'success');
      replaceInput.focus();
    }
    findInput.addEventListener('input', function () { rebuildMatches(0); });
    textarea.addEventListener('input', function () { rebuildMatches(textarea.selectionStart); });
    caseButton.addEventListener('click', function () {
      var active = !isCaseSensitive();
      caseButton.setAttribute('aria-pressed', active ? 'true' : 'false');
      caseButton.classList.toggle('is-active', active);
      rebuildMatches(0);
    });
    ...
    rebuildMatches(0);
    editor.beforeClose = function () {
      if (textarea.value === localEditor.preText) return true;
      return window.confirm('Existem alterações não salvas. Deseja descartá-las?');
    };
